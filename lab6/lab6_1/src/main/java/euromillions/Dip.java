...
public class Dip {
    ...
    private static int NUMBERS_MIN_VALUE = 1;
    private static int NUMBERS_MAX_VALUE = 50;

    private static int STARS_COUNT = 2;
    private static int STARS_MIN_VALUE = 1;
    private static int STARS_MAX_VALUE = 12;
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if (NUMBERS_COUNT == arrayOfNumbers.length && STARS_COUNT == arrayOfStarts.length) {
            ...
            for (int number : arrayOfNumbers) if (number < NUMBERS_MIN_VALUE || number > NUMBERS_MAX_VALUE) throw new IllegalArgumentException();
            for (int start : arrayOfStarts) if (start < STARS_MIN_VALUE || start > STARS_MAX_VALUE) throw new IllegalArgumentException();
        } else {
